...
Qx1 = [ ...
    ...
    70, 70, 70];      % spacing/aero 1-2, 2-3 and 3-4
...
%==========================================================================
% N = 35 intervalli, 36 nodi, orizzonte totale 50 s.
%
% La griglia e':
% - piu' fitta vicino a t = 0;
% - piu' rada al centro;
% - piu' fitta vicino a t = 50 s.
%
...
Ts      = Ts_ctrl;  % compatibilita' con i blocchi che usano ancora Ts
...
% Tempi cumulativi dei nodi, vettore colonna
...
if abs(dt_grid(1)-Ts_ctrl) > 1e-10
    error('Il primo intervallo non e'' pari a Ts_ctrl.');
end
...
% NOTA: gap iniziale veicolo 3-4 replicato dallo stesso passo (10 m) usato
% per gli altri veicoli del platoon (-9, -19 -> -29).
