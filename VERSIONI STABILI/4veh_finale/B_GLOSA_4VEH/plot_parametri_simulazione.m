...
%% --------------------------------------------------------
required = {'N','N_near','N_far','Ts_near','Ts_far','Ts_ctrl','T_hor', ...
...
% Pesi dalla diagonale di W1 (Qx1, 23 termini):
% [1-3]=dist, [4-6]=jerk, [7-9]=Ax, [10-12]=stop_tgt,
% [13-15]=dwell_pos, [16-18]=dwell_vel, [19-21]=dwell_acc, [22-23]=aero
w_dist      = Qx1(1);
w_jerk      = Qx1(4);
w_Ax        = Qx1(7);
w_stop_tgt  = Qx1(10);
...
% Levenberg-Marquardt: da ACADO_3veh_4TL.m (non in workspace), valore fisso
...
fprintf("\n%s\n", sep);
fprintf("  PARAMETRI SIMULAZIONE  -  B-GLOSA  3VEH  4TL\n");
...
fprintf("  eps_aero       = %.2f m^2   smoothing costo aerodinamico\n", eps_aero);
fprintf("\n");
...
fprintf("%s\n [6] PESI COST FUNCTION  (letti da Qx1)\n%s\n", sep2, sep2);
...
fprintf("  w_aero         = %7.2f    spacing/aerodinamico (coppie 1-2, 2-3)\n", w_aero);
...
title(sprintf("Griglia predittiva non uniforme    N=%d,  T_{hor}=%.1f s,  Ts_{ctrl}=%.1f s", N, T_hor, Ts_ctrl), "FontSize", 10);
...
lww = {"dist","jerk","Ax","stop tgt","dwell pos","dwell vel","dwell acc","aero","dist(N)"};
wws = [w_dist, w_jerk, w_Ax, w_stop_tgt, w_dwell_pos, w_dwell_vel, w_dwell_acc, w_aero, w_dist_N];
clw = [cB;cP;cA;cG;cR;cR;cR;cK;cB];
bw = bar(wws, 0.7, "FaceColor", "flat"); bw.CData = clw;
axw = gca; axw.XTick = 1:length(lww); axw.XTickLabel = lww; axw.XTickLabelRotation = 25;
ylabel("Peso Q  [-]"); title("Pesi cost function  (da Qx1 e QN1 del workspace)", "FontSize", 10);
grid on; box on;
for i = 1:length(wws)
...
sn   = (gvec - d_gap - sqrt((gvec - d_gap).^2 + eps_aero)) / 2;
ca   = (sn / d_gap).^2*100;
plot(gvec, ca, "Color", cB, "LineWidth", 2); hold on;
fill([0 gap_min gap_min 0], [-0.65 -0.65 0.05 0.05], cR, "FaceAlpha", 0.12, "EdgeColor", "none");
xline(gap_min, ":", "Color", cR, "LineWidth", 1.5);
xline(d_gap,   "--", "Color", cA, "LineWidth", 1.5);
text(gap_min+0.15, -0.57, sprintf("gap_{min} = %.0f m", gap_min), "Color", cR, "FontSize", 9);
text(d_gap+0.15,   -0.47, sprintf("d_{gap} = %.0f m",  d_gap),   "Color", cA, "FontSize", 9);
xlabel("Gap netto  [m]"); ylabel("cost\_aero  [-]");
title(sprintf("Costo spacing/aero    L_{veh}=%.1f m,  d_{gap}=%.0f m,  gap_{min}=%.0f m,  \\epsilon_{aero}=%.2f", ...
    L_platoon, d_gap, gap_min, eps_aero), "FontSize", 10);
grid on; box on; ylim([-0.65 0.05]);
