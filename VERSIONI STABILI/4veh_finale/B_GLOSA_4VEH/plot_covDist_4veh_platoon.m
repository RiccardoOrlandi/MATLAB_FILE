function h = plot_covDist_4veh_platoon(out, i_fig, s_TL, s_stop, L_platoon)
% ============================================================
% PLOT_COVDIST_4VEH_PLATOON
% Genera 3 figure:
%   Fig i_fig   - Traiettoria spazio-tempo + fasi TL + fermate
%   Fig i_fig+1 - Jerk / Velocita' / Accelerazione (4 veicoli sovrapposti)
...
%% ----------------------------------------------------------
%  2. ESTRAZIONE CONTROLLI (u, u2, u3, u4 = struct [1x1xN])
%  ----------------------------------------------------------

t_u = out.u.time(:);
u1  = squeeze(out.u.signals.values);        % [N x 1] -> [N]
u2  = squeeze(out.u2.signals.values);
u3  = squeeze(out.u3.signals.values);
u4  = squeeze(out.u4.signals.values);
...
hh(7) = plot(t_st, s4,  '-',  'Color', cV4, 'LineWidth', lw);
hh(8) = plot(t_st, s4t, '--', 'Color', cV4, 'LineWidth', lw);

grid on; box on;
xlabel('Tempo [s]');  ylabel('Posizione [m]');
legend(hh, {'Veh 1 testa','Veh 1 coda', ...
            'Veh 2 testa','Veh 2 coda', ...
            'Veh 3 testa','Veh 3 coda', ...
            'Veh 4 testa','Veh 4 coda'}, 'Location', 'best');
...
plot(ax_j, t_u, u4, '-', 'Color', cV4, 'LineWidth', lw, 'DisplayName', 'Veh 4');
...
plot(ax_v, t_st, v4.*3.6, '-', 'Color', cV4, 'LineWidth', lw, 'DisplayName', 'Veh 4');
...
plot(ax_a, t_st, a4, '-', 'Color', cV4, 'LineWidth', lw, 'DisplayName', 'Veh 4');
...
sgtitle(tl2, 'Cinematica – 4 veicoli sovrapposti');
