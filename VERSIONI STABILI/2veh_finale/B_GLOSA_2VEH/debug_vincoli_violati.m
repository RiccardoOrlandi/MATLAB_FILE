...
%   - ricostruire tutti i vincoli dell'OCP ACADO 2 veicoli / 4 TL attivi
...
function score = onlineDataScore(OD)
    ...
    % Vmax dei due veicoli devono essere positivi e ragionevoli.
    Vmax1 = OD(:,10);
    Vmax2 = OD(:,31);
    ...
    % k_road dei due veicoli devono essere finiti e non enormi.
    kroad1 = OD(:,9);
    kroad2 = OD(:,30);
...
function [Tviol_max, Tviol_node, Tall_max] = computeConstraintResiduals2veh(X, U, OD, tol)
    ...
    %%=====================================================================
    % Parametri coerenti con ACADO_2veh_4TL.m
    %======================================================================
    ...
    V_max_static = 50/3.6;
    d_safe_TL = 0;
    v_eps = 0.02;          % allineato ad ACADO_2veh_4TL.m

    gap_min = 1;           % allineato ad ACADO_2veh_4TL.m

    eps_stop_num = 0.1;    % tolleranza numerica fermata [m]
    ...
    function addStopConstraints(id,pos_i,vel_i,acc_i,D)
        ...
        % Stop approach
        % Vincolo ACADO:
        % pos_i <= (s_stop+eps_stop_num)*(1-x_stop) + s_max*x_stop
        %--------------------------------------------------------------

        r_stop = pos_i - ...
                 ((D.s_stop + eps_stop_num) .* (1 - D.x_stop) + s_max .* D.x_stop);
