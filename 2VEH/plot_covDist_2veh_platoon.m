...
% ============================================================
% PLOT_COVDIST_2VEH_PLATOON
% Genera 3 figure:
%   Fig i_fig   - Traiettoria spazio-tempo + fasi TL + fermate
%   Fig i_fig+1 - Jerk / Velocita' / Accelerazione (2 veicoli sovrapposti)
...
xs   = out.states.signals.values;

if size(xs,2) < 6
    error('out.states deve avere almeno 6 colonne: [s1 v1 a1 s2 v2 a2].');
end
...
%% ----------------------------------------------------------
%  2. ESTRAZIONE CONTROLLI (u, u2 = struct [1x1xN])
%  ----------------------------------------------------------
...
legend(hh, {'Veh 1 testa','Veh 1 coda', ...
            'Veh 2 testa','Veh 2 coda'}, 'Location', 'best');
...
sgtitle(tl2, 'Cinematica – 2 veicoli sovrapposti');
