%==========================================================================
% ACADO NMPC - B-GLOSA 4 veicoli indipendenti - 4 TL attivi
%==========================================================================
% Script per generazione/export solver NMPC ACADO.
%
% Estensione della versione 3 veicoli (3VEH_40s_nonuniform):
% - aggiunge stati [pos4, vel4, acc4];
% - aggiunge controllo jerk4;
% - estende OnlineData da 63 a 84 elementi per nodo (21 per veicolo x 4);
% - replica vincoli semaforici, fermate, velocita' e jerk per il quarto
%   veicolo;
% - aggiunge vincolo e costo di spacing tra veicolo 3 e veicolo 4.
%=========================================================================
clc;
clear all;
close all;
...
% veicolo 4: pos4, vel4, acc4
DifferentialState pos vel acc pos2 vel2 acc2 pos3 vel3 acc3 pos4 vel4 acc4;

% Ingressi di controllo: jerk longitudinale di ciascun veicolo
Control jerk jerk2 jerk3 jerk4;

% OnlineData per nodo: 84 = 21 dati per veicolo.
% Per ciascun veicolo:
...
OnlineData x_TL1 x_TL2 x_TL3 x_TL4 ...
    ...
    x_TL1_4 x_TL2_4 x_TL3_4 x_TL4_4 ...
    s_stop_active_4 x_stop_active_4 x_dwell_active_4 w_stop_active_4 k_road_4 Vmax_4 dt_schd_4 s_st_4 s_hor_4 ...
    x_TL1_tail_4 x_TL2_tail_4 x_TL3_tail_4 x_TL4_tail_4 ...
    s_TL1_active_4 s_TL2_active_4 s_TL3_active_4 s_TL4_active_4;
...
% cost_dist4 = is(s_max - pos4) / s_max;
cost_dist = is((1 - x_dwell_active) * (s_max - pos) / s_max);
cost_dist2 = is((1 - x_dwell_active_2) * (s_max - pos2) / s_max);
cost_dist3 = is((1 - x_dwell_active_3) * (s_max - pos3) / s_max);
cost_dist4 = is((1 - x_dwell_active_4) * (s_max - pos4) / s_max);

cost_jerk  = is(jerk)  / jerk_max;
cost_jerk2 = is(jerk2) / jerk_max;
cost_jerk3 = is(jerk3) / jerk_max;
cost_jerk4 = is(jerk4) / jerk_max;

cost_Ax  = is(acc)  / Ax_max;
cost_Ax2 = is(acc2) / Ax_max;
cost_Ax3 = is(acc3) / Ax_max;
cost_Ax4 = is(acc4) / Ax_max;
...
smooth_neg34 = (is(gap34 - d_gap) - sqrt(is((gap34 - d_gap)*(gap34 - d_gap)) + eps_aero)) / 2;

cost_aero12 = is(smooth_neg12 / d_gap);
cost_aero23 = is(smooth_neg23 / d_gap);
cost_aero34 = is(smooth_neg34 / d_gap);
...
cost_stop_target4 = is(w_stop_active_4 * (pos4 - s_stop_active_4) / s_max);
...
cost_dwell_pos4 = is(x_dwell_active_4 * (pos4 - s_stop_active_4) / eps_stop_front);
...
cost_dwell_vel4 = is(x_dwell_active_4 * vel4 / eps_stop_v);
...
cost_dwell_acc4 = is(x_dwell_active_4 * acc4 / eps_stop_a);
...
%  5-8   jerk 1,2,3,4
%  9-12  Ax 1,2,3,4
%  13-16 stop target 1,2,3,4
%  17-20 dwell position 1,2,3,4
%  21-24 dwell velocity 1,2,3,4
%  25-28 dwell acceleration 1,2,3,4
%  29-31 spacing/aero 1-2, 2-3, 3-4
%--------------------------------------------------------------------------

h = {cost_dist; cost_dist2; cost_dist3; cost_dist4; ...
     cost_jerk; cost_jerk2; cost_jerk3; cost_jerk4; ...
     cost_Ax; cost_Ax2; cost_Ax3; cost_Ax4; ...
     cost_stop_target; cost_stop_target2; cost_stop_target3; cost_stop_target4; ...
     cost_dwell_pos; cost_dwell_pos2; cost_dwell_pos3; cost_dwell_pos4; ...
     cost_dwell_vel; cost_dwell_vel2; cost_dwell_vel3; cost_dwell_vel4; ...
     cost_dwell_acc; cost_dwell_acc2; cost_dwell_acc3; cost_dwell_acc4; ...
     cost_aero12; cost_aero23; cost_aero34};

hN = {cost_dist; cost_dist2; cost_dist3; cost_dist4};
...
% Griglia predittiva non uniforme: 44 intervalli, orizzonte totale 50 s
% (identica alla versione 3 veicoli)
% - intervalli 1..14 : 1.0 s  -> [0, 14] s
% - intervalli 15..44: 1.2 s  -> [14, 50] s
N_near  = 14;
N_far   = 30;
Ts_near = 1.0;
Ts_far  = 1.2;
...
ocp.subjectTo(acc4 <= Ax_max);
ocp.subjectTo(acc4 >= Ax_min);
...
ocp.subjectTo(jerk4 <= jerk_max);
ocp.subjectTo(jerk4 >= jerk_min);
...
ocp.subjectTo(is(vel4 * vel4 * k_road_4 - Ay_max) <= 0);
...
ocp.subjectTo(is(pos4 - ((s_stop_active_4 + eps_stop_num)*(1-x_stop_active_4) + s_max*x_stop_active_4)) <= 0);
...
ocp.subjectTo(is(pos4 - ((s_stop_active_4 + eps_stop_front)*x_dwell_active_4 + s_max*(1-x_dwell_active_4))) <= 0);
ocp.subjectTo(is((s_stop_active_4 - eps_stop_back)*x_dwell_active_4 - s_max*(1-x_dwell_active_4) - pos4) <= 0);
...
ocp.subjectTo(is(vel4 - (eps_stop_v*x_dwell_active_4 + V_max*(1-x_dwell_active_4))) <= 0);
ocp.subjectTo(is(acc4 - (eps_stop_a*x_dwell_active_4 + Ax_max*(1-x_dwell_active_4))) <= 0);
ocp.subjectTo(is(acc4 + (eps_stop_a*x_dwell_active_4 + abs(Ax_min)*(1-x_dwell_active_4))) >= 0);
...
% Testa veicolo 4
ocp.subjectTo(is(pos4 - ((s_TL1_active_4-d_safe_TL)*(1-x_TL1_4) + s_max*x_TL1_4)) <= 0);
ocp.subjectTo(is(pos4 - ((s_TL2_active_4-d_safe_TL)*(1-x_TL2_4) + s_max*x_TL2_4)) <= 0);
ocp.subjectTo(is(pos4 - ((s_TL3_active_4-d_safe_TL)*(1-x_TL3_4) + s_max*x_TL3_4)) <= 0);
ocp.subjectTo(is(pos4 - ((s_TL4_active_4-d_safe_TL)*(1-x_TL4_4) + s_max*x_TL4_4)) <= 0);

% Coda veicolo 4
ocp.subjectTo(is((pos4-L_platoon) - ((s_TL1_active_4-d_safe_TL)*(1-x_TL1_tail_4) + s_max*x_TL1_tail_4)) <= 0);
ocp.subjectTo(is((pos4-L_platoon) - ((s_TL2_active_4-d_safe_TL)*(1-x_TL2_tail_4) + s_max*x_TL2_tail_4)) <= 0);
ocp.subjectTo(is((pos4-L_platoon) - ((s_TL3_active_4-d_safe_TL)*(1-x_TL3_tail_4) + s_max*x_TL3_tail_4)) <= 0);
ocp.subjectTo(is((pos4-L_platoon) - ((s_TL4_active_4-d_safe_TL)*(1-x_TL4_tail_4) + s_max*x_TL4_tail_4)) <= 0);
