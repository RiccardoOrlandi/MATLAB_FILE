...
% eps_aero = 0.1;  % [m^2] smoothing pseudo-Huber (differenziabilita' in gap=d_gap)
% gap_min  = 1;    % [m] gap minimo assoluto (vincolo hard)
% gap_off  = 4;    % [m] soglia oltre cui il costo si azzera (veicoli lontani ignorati)
% eps_win  = 0.1;  % [m^2] smoothing finestra (transizione completa in ~2m oltre gap_off)

%--------------------------------------------------------------------------
% Costo spacing con FINESTRA DI ATTIVAZIONE.
%
% Struttura: f(gap) = huber(gap) * window(gap) / d_gap
%
% 1) huber(gap) = sqrt((gap-d_gap)^2 + eps_aero) - sqrt(eps_aero)
%    Pseudo-Huber centrata in d_gap:
%    - vale 0 in gap=d_gap (minimo esatto)
%    - gradiente nullo in d_gap (C-inf, compatibile Gauss-Newton)
%    - cresce simmetricamente per gap < d_gap e gap > d_gap
%
% 2) window(gap) = [smooth_pos(gap_off - gap)]^2 / [smooth_pos(gap_off - d_gap)]^2
%    dove smooth_pos(x) = (x + sqrt(x^2 + eps_win)) / 2
%    - vale esattamente 1 in gap=d_gap (normalizzata)
%    - decade a ~0 attorno a gap_off (finestra di spegnimento)
%    - sempre >= 0 (nessun costo negativo)
%    - il quadrato rende la decadenza piu ripida (1/x^2 invece di 1/x)
%
% Effetto pratico:
%   gap < d_gap         -> costo positivo crescente (penalizza avvicinamento)
%   gap = d_gap = 3m    -> costo = 0 (ottimo, il solver ci rimane)
%   d_gap < gap < gap_off -> costo positivo decrescente (incentivo a stare a d_gap)
%   gap >= gap_off = 7m -> costo ~ 0 (veicoli lontani: nessun incentivo ad avvicinarsi)
%--------------------------------------------------------------------------

% Pseudo-Huber (termine di forma)
% ph_12   = is(gap12 - d_gap);
% ph_23   = is(gap23 - d_gap);
% huber12 = sqrt(is(ph_12 * ph_12) + eps_aero) - sqrt(eps_aero);
% huber23 = sqrt(is(ph_23 * ph_23) + eps_aero) - sqrt(eps_aero);
% 
% % Finestra smooth (spegnimento oltre gap_off)
% % Costante di normalizzazione calcolata in d_gap (valore numerico fisso)
% c_win = ((gap_off - d_gap) + sqrt((gap_off - d_gap)^2 + eps_win)) / 2;
% 
% win_raw12 = (is(gap_off - gap12) + sqrt(is((gap_off - gap12)*(gap_off - gap12)) + eps_win)) / 2;
% win_raw23 = (is(gap_off - gap23) + sqrt(is((gap_off - gap23)*(gap_off - gap23)) + eps_win)) / 2;
% 
% % Finestra normalizzata al quadrato: vale 1 in d_gap, ~0 oltre gap_off
% win12 = is((win_raw12 * win_raw12) / (c_win * c_win));
% win23 = is((win_raw23 * win_raw23) / (c_win * c_win));
% 
% cost_aero12 = is(huber12 * win12 / d_gap);
% cost_aero23 = is(huber23 * win23 / d_gap);
%--------------------------------------------------------------------------
d_gap    = 3;    % gap netto desiderato [m]
...
mpc.set('NUM_INTEGRATOR_STEPS',        N);   % 2 sub-step RK4 per intervallo (sufficiente per triple-integrator lineare)
