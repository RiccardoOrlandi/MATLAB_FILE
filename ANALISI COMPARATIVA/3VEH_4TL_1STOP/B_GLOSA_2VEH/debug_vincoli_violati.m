%%=========================================================================
% DEBUG VINCOLI NMPC 3 VEH - SENZA AGGIUNGERE TO WORKSPACE
%=========================================================================
...
%
% Obiettivo:
%   - trovare lo step in cui status == target_status
...
idx_user      = [];     % lascia [] per primo status == target_status, oppure imposta indice manuale
...
%%=========================================================================
% 3. TROVA STEP CON STATUS == target_status
%==========================================================================
...
function score = onlineDataScore(OD)
    ...
    % Blocco veicolo: [1:4, 6:8, 14:17] rispetto al blocco locale.
    flag_cols = [];
    for base = [0 21 42]
        flag_cols = [flag_cols, base + 1:base + 4, base + 6:base + 8, base + 14:base + 17]; %#ok<AGROW>
    end
...
function [Tviol_max, Tviol_node, Tall_max] = computeConstraintResiduals3veh(X, U, OD, tol)
    ...
    addConstraint('veh3 vel3^2*k_road_3 <= Ay_max', vel3.^2 .* D3.k_road - Ay_max);
    ...
    addConstraint('gap12 ', gap_min - gap12);
    addConstraint('gap23', gap_min - gap23);
    ...
    function addStopConstraints(id,pos_i,vel_i,acc_i,D)
        ...
        r_stop = pos_i - (D.s_stop .* (1 - D.x_stop) + s_max .* D.x_stop);

        addConstraint([prefix ' stop approach'], r_stop);

        r_dwell_front = pos_i - ((D.s_stop + eps_stop_front) .* D.x_dwell + ...
                                  s_max .* (1 - D.x_dwell));

        r_dwell_back = (D.s_stop - eps_stop_back) .* D.x_dwell - ...
        ...
        r_dwell_vel = vel_i - (eps_stop_v .* D.x_dwell + ...
                               V_max_static .* (1 - D.x_dwell));

        r_dwell_acc_max = acc_i - (eps_stop_a .* D.x_dwell + ...
                                   Ax_max .* (1 - D.x_dwell));

        r_dwell_acc_min = -(acc_i + (eps_stop_a .* D.x_dwell + ...
    ...
    function addTLConstraints(id,pos_i,D)
        ...
        for kk = 1:4
            ...
            r_head = pos_i - ((sTL - d_safe_TL) .* (1 - D.x_TL(:,kk)) + ...
                              s_max .* D.x_TL(:,kk));
